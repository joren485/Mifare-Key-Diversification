...
UID="F4EA548E"
Sector_number="05"
MIFkey="A0A1A2A3A4A5"## aleen nodig bij 3des key div(MF RC171)
#########
##niet bewerken
#########
def gen_subkeys(K,cipher):
    """genereer subkeys van cipher"""
...
def cmac_div(key,UID,Sector_number):
	...
	xordata=M[-32:].decode("hex")##laatste 16 bytes van M
	xoreddata=xor(xordata,xorkey)##xor xordata met K1 of K2
	M=M[:-32]+xoreddata##vervang laatste 16 bytes door xordata

	cipher = AES.new(key.decode("hex"),AES.MODE_CBC,IV)##reset cipher
	divkey=cipher.encrypt(M.decode("hex")).encode("hex")[-32:-20]##aes M voor CMAC en index meteen het goede stuk
